...
@interface WordGridHelpViewController ()

@end

@implementation WordGridHelpViewController

@synthesize delegate, textBackgroundView, stars, starIdx, starToggle;
...
- (void)viewDidLoad
{
    ...
    upSwipe.delaysTouchesBegan = YES;
    ...
    downSwipe.delaysTouchesBegan = YES;
...
{
    [self.delegate wordGridHelpDidFinish:self];
}

- (void)swipedDown
{
    if (self.textBackgroundView.frame.origin.y >= 80) {
        return;
    }
    [UIView animateWithDuration:0.3 animations:^{
        self.textBackgroundView.center = CGPointMake(self.textBackgroundView.center.x, self.textBackgroundView.center.y + 100);
    }];
...
- (void)swipedUp
{
    if (self.textBackgroundView.frame.origin.y < -50) {
        return;
    }
    [UIView animateWithDuration:0.3 animations:^{
       self.textBackgroundView.center = CGPointMake(self.textBackgroundView.center.x, self.textBackgroundView.center.y - 100);
